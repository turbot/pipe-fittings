...
func (c *DbClient) startQueryWithRetries(ctx context.Context, session *db_common.DatabaseSession, query string, args ...any) (*sql.Rows, error) {
	...
	err := retry.Do(ctx, retry.WithMaxDuration(maxDuration, backoff), func(ctx context.Context) error {
		...
		// TODO KAI not supported by steampipe
		...
		//connectionStateMap, stateErr := modconfig.LoadConnectionState(ctx, sysConn.Conn(), modconfig.WithWaitUntilLoading())
		//if stateErr != nil {
		...
		//	if _, err := modconfig.LoadConnectionState(ctx, sysConn.Conn(), modconfig.WithWaitForSearchPath(searchPath)); err != nil {
		//		return err
		//	}
		//
		...
		//statusMessage := modconfig.GetLoadingConnectionStatusMessage(connectionStateMap, missingSchema)
