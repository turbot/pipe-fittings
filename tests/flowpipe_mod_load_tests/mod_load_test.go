...
func (suite *FlowpipeModLoadTestSuite) TestInputStepContainsNotifyBlockThatHasVarOnIt() {
	...
	workspace, errorAndWarning := workspace.LoadWithParams(suite.ctx, "./mods/step_with_notify_and_var_default", map[string]modconfig.Credential{}, map[string]modconfig.CredentialImport{}, ".hcl", ".sp")
...
func (suite *FlowpipeModLoadTestSuite) TestNotifyDependsAnotherStep() {
	...
	workspace, errorAndWarning := workspace.LoadWithParams(suite.ctx, "./mods/notify_depends_another_step", map[string]modconfig.Credential{}, map[string]modconfig.CredentialImport{}, ".hcl", ".sp")
	assert.Nil(errorAndWarning.Error)
...
func (suite *FlowpipeModLoadTestSuite) TestNotifyWithRuntimeParam() {
	...
	workspace, errorAndWarning := workspace.LoadWithParams(suite.ctx, "./mods/notify_with_runtime_param", map[string]modconfig.Credential{}, map[string]modconfig.CredentialImport{}, ".hcl", ".sp")
	assert.Nil(errorAndWarning.Error)
