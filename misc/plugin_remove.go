...
func (r PluginRemoveReports) Print() {
	...
	if length > 0 {
		fmt.Printf("\nUninstalled %s:\n", utils.Pluralize("plugin", length)) //nolint:forbidigo // TODO: fix this
		for _, report := range r {
			org, name, _ := report.Image.GetOrgNameAndStream()
			fmt.Printf("* %s/%s\n", org, name) //nolint:forbidigo // TODO: fix this
		...
		fmt.Println() //nolint:forbidigo // TODO: fix this
		...
		if staleLength > 0 {
			...
			fmt.Println(strings.Join(str, "\n")) //nolint:forbidigo // TODO: fix this
		}
	}
}
