...
func TestAzureDefaultCredential(t *testing.T) {
	...
	os.Setenv("AZURE_CLIENT_SECRET", "gdget3464tergrgsdf")
	os.Setenv("AZURE_ENVIRONMENT", "AzureUSGovernment")
	os.Setenv("AZURE_TENANT_ID", "gdgdghdfhdhfh")
	...
	assert.Equal("AzureUSGovernment", *newAzureCreds.Environment)
	assert.Equal("gdgdghdfhdhfh", *newAzureCreds.TenantID)
}

func TestAzureDefaultCredentialWithoutEnvironmentConfigured(t *testing.T) {
	assert := assert.New(t)
