...
func Remove(ctx context.Context, image string, pluginConnections map[string][]*modconfig.Connection) (*misc.PluginRemoveReport, error) {
	...
	return &misc.PluginRemoveReport{Connections: conns, Image: imageRef}, err
}
		...
		if found {
			...
			if !detectLocalPlugin(installation, pluginBinary) {
				...
				if err != nil {
					...
					return nil, fmt.Errorf("could not evaluate plugin version %s", installation.Version)
				}
			}
